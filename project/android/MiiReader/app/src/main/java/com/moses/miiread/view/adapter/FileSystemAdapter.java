...
import com.moses.miiread.view.adapter.base.IViewHolder;
import com.moses.miiread.view.adapter.view.FileHolder;
...
public class FileSystemAdapter extends BaseListAdapter<File> {
    ...
    public void uncheckItems(List<File> values) {
        for (File file : values)
            mCheckMap.put(file, false);
        notifyDataSetChanged();
    }

    public void uncheckItem(File file) {
        mCheckMap.put(file, false);
        notifyDataSetChanged();
    }
    ...
    public boolean getItemIsChecked(int pos) {
        File file = getItem(pos);
        Boolean selected = mCheckMap.get(file);
