...
import com.moses.miiread.view.popupwindow.CheckAddShelfPop;
import com.moses.miiread.view.popupwindow.ReadBottomMenu;
...
import com.moses.miiread.widget.page.PageLoader;
import com.moses.miiread.widget.page.PageView;
...
public class ReadBookActivity extends MBaseActivity<ReadBookContract.Presenter> implements ReadBookContract.View {
    ...
    private int curChapterPosi = -1;

    private AlertDialog readStylePop;
    ...
    protected void initImmersionBar() {
        ...
        if (readBookControl.getHideNavigationBar()) {
            ...
            if (ImmersionBar.hasNavigationBar(this)) {
                llMenuBottom.setNavigationBar(!readBookControl.getFixBottomNav() ? ImmersionBar.getNavigationBarHeight(this) : 0);
            }
        }
    ...
    private void initBottomMenu() {
        llMenuBottom.setListener(new ReadBottomMenu.OnMenuListener() {
            ...
            public void download() {
                ReadBookActivity.this.download();
            }
            ...
            public void openMoreSetting() {
                //设置
                SettingActivity.startThis(ReadBookActivity.this);
                mHandler.postDelayed(ReadBookActivity.this::popMenuOut, 400);
            }
    ...
    private void showReadInterfacePop() {
        try {
            ReadInterfacePopMoses readInterfacePop = new ReadInterfacePopMoses(this);
            readInterfacePop.setId(R.id.pop);
            readInterfacePop.setListener(this, new ReadInterfacePopMoses.OnChangeProListener() {
                ...
                public void bgChange() {
                    ReadBookActivity.this.bgChange();
                }
            ...
            readStylePop = new AlertDialog.Builder(this, R.style.alertDialogTheme)
                    .setView(readInterfacePop)
                    .create();
            readStylePop.setCanceledOnTouchOutside(true);
            readStylePop.setCancelable(true);
            readStylePop.show();
            Window window = readStylePop.getWindow();
            WindowManager.LayoutParams params;
            if (window != null) {
                readStylePop.getWindow().setDimAmount(0f);
                params = window.getAttributes();
                int sBarHeight = ScreenUtils.getStatusBarHeight();
                int[] screenWH = ScreenUtils.getScreenSize(this);
                params.width = (int) (0.8f * Math.min(screenWH[0], screenWH[1]));
                params.x = screenWH[0] - 2 * sBarHeight - ScreenUtils.dpToPx(8);
                params.y = 0;
    ...
    protected void bindEvent() {
        ll_origin.setOnClickListener(view -> {
            boolean onLine = mPresenter.getBookShelf() != null && !mPresenter.getBookShelf().getTag().equals(BookShelfBean.LOCAL_TAG);
            //打开URL
            if (onLine && ConfigMng.JUMP_TO_SOURCE_ENABLE) {
                try {
                    if (curChapterPosi != -1) {
                        String url = mPresenter.getBookShelf().getChapter(curChapterPosi).getDurChapterUrl();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id) {
            case R.id.action_interface:
                //界面
                ReadBookActivity.this.popMenuOut();
                mHandler.postDelayed(ReadBookActivity.this::showReadInterfacePop, menuBottomOut.getDuration() + 100);
                break;
    ...
    private void changeSource() {
        ...
        mHandler.postDelayed(ReadBookActivity.this::popMenuOut, 200);
        if (mPresenter.getBookShelf() != null) {
    ...
    @Override
    public void refreshOriginTxt() {
        String sourceName;
        if (mPresenter.getBookSource() == null)
            sourceName = getResources().getString(R.string.source_is_deleted);
        else
            sourceName = StringUtils.fixSourceDisplayName(mPresenter.getBookShelf().getBookInfoBean().getOrigin());
        tvSourceFrom.setText(String.format(getResources().getString(R.string.source_from_fmt), sourceName));
    }
    ...
    public void upMenu() {
        ...
        if (onLine) {
            ...
            llMenuBottom.findViewById(R.id.change_origin_ll).setVisibility(View.VISIBLE);
        } else {
            tvSourceFrom.setVisibility(View.GONE);
            llMenuBottom.findViewById(R.id.change_origin_ll).setVisibility(View.GONE);
        }
    ...
    @Override
    public void updateReadStyleDlgTone() {
        if (readStylePop != null) {
            ReadInterfacePopMoses interfacePopMoses = readStylePop.findViewById(R.id.pop);
            interfacePopMoses.updateBgV();
        }
    }
